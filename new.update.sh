...
Show() {
    # OK
    if (($1 == 0)); then
        echo -e "- OK $2" >> /var/log/casaos/upgrade.log
    # FAILED
    elif (($1 == 1)); then
        echo -e "- FAILED $2" >> /var/log/casaos/upgrade.log
    # INFO
    elif (($1 == 2)); then
        echo -e "- INFO $2" >> /var/log/casaos/upgrade.log
    # NOTICE
    elif (($1 == 3)); then
        echo -e "- NOTICE $2" >> /var/log/casaos/upgrade.log
    fi
}
...
Check_Arch() {
    ...
    *)
	...
	"https://github.com/zhanghengxin/CasaOS-UI/releases/download/v0.3.6/linux-all-casaos-0.3.6-alpha.tar.gz"
)
}
